...
jobposts_df = pd.DataFrame(list(jobposts_colle.find()))
resumes_df = pd.DataFrame(list(resumes_colle.find()))
...
@app.route("/cvsearch/<id>")
def findResForJob(id):
    results = []

    row = jobposts_df[jobposts_df["_id"] == id]
    # # print(row.index[0])
    # print(type(jobposts_tfidf))
    # print((jobposts_tfidf))
    # print("testttt", jobposts_tfidf[row.index[0]])
